	...
	handler := func() error {
		...
		if isCreate {
			if task.fetcherManager.AutoRename() {
				...
				task.Meta.Opts.Name = util.SafeFilename(task.Meta.Opts.Name, util.MaxFilenameLength)
				// check if the download file is duplicated and rename it automatically.
				if task.Meta.Res.Name != "" {
					task.Meta.Res.Name = util.SafeFilename(task.Meta.Res.Name, util.MaxFilenameLength)
					...
					task.Meta.Res.Files[0].Name = util.SafeFilename(task.Meta.Res.Files[0].Name, util.MaxFilenameLength)
