...
typedef HostCreateHandler = Future<void> Function(CreateTask createTask, bool silent);

typedef HostForwardHandler =
    Future<dynamic> Function({required String path, required String method, dynamic data, Map<String, dynamic>? query});
...
class HostRpcService {
  ...
  Future<void> start({required HostCreateHandler onCreate, required HostForwardHandler onForward}) async {
    if (_server != null) {
      return;
    }
      ...
      routes: {
        ...
        '/forward': (ctx) async {
          try {
            ...
            final response = await onForward(path: path, method: method, data: data, query: query);
